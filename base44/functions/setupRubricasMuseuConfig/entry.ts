...
function getRubricaCentroCusto(rubrica: any): string {
  return normalizeMuseu(
    rubrica?.centro_custo || rubrica?.museu || rubrica?.museu_codigo || rubrica?.unidade || ''
  );
}

function getBudgetLineCentroCusto(budgetLine: any): string {
  return normalizeMuseu(
    budgetLine?.centro_custo || budgetLine?.museu || budgetLine?.museu_codigo || budgetLine?.unidade || ''
  );
}
...
function resolverMuseusDaRubrica(rubrica: any, budgetLine: any = null) {
  ...
  if (rubricaMuseu && MUSEUS.includes(rubricaMuseu)) {
    return { museus: [rubricaMuseu], origem: 'rubrica_centro_custo' };
  }
  const budgetMuseu = getBudgetLineCentroCusto(budgetLine);
  if (budgetMuseu && MUSEUS.includes(budgetMuseu)) {
    return { museus: [budgetMuseu], origem: 'budgetline_centro_custo' };
  }
  ...
  if (heuristica.length > 0) {
    return { museus: Array.from(new Set(heuristica)), origem: 'heuristica_textual' };
  }
  return { museus: [], origem: 'nao_resolvido' };
}
...
Deno.serve(async (req) => {
  try {
    ...
    for (const rubrica of rubricas.filter((r) => r?.ativo !== false)) {
      const budgetlineId =
        rubrica?.budgetline_id || rubrica?.budget_line_id || rubrica?.linha_orcamentaria_id || null;
