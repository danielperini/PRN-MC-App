// ================================================================
// lerNotaFiscalGPT
// Leitura integral de Notas Fiscais com OpenAI (gpt-4o) usando
// Structured Outputs (strict JSON Schema) e PDF via Files API.
//
// Regras principais:
//   - A chave da OpenAI NUNCA é exposta ao frontend; toda chamada
//     acontece neste backend Base44.
//   - Hierarquia de fontes: XML fiscal -> PDF/DANFE -> Compra vinculada
//                              -> Rubricas/Metas oficiais -> Nome do arquivo.
//   - Data de emissão é SOMENTE a data fiscalmente rotulada; jamais
//     data de abertura/constituição/inscrição no CNPJ/etc.
//   - Não altera fluxo de aprovação, saldos, compras/rubricas existentes,
//     notificações, backups ou documentos já revisados manualmente.
//   - Resultado: structured JSON com evidências, alertas, score e
//     status_revisao. Não grava nada — apenas devolve o laudo.
// ================================================================
...
function isISODate(d) {
  return /^\d{4}-\d{2}-\d{2}$/.test(safeStr(d));
}
...
// ─── Parse XML raw (determinístico, prioridade sobre IA) ────────
...
// ─── Download de arquivo (direto OU via conector Google Drive) ─
async function resolverDownload(base44, url) {
  if (!url || typeof url !== 'string') throw new Error('URL inválida');
  const u = url.trim();

  // Google Drive "view" URL: precisa do file_id e do access token OAuth.
  ...
  // URL direta disponível (CDN pública, signed URL, etc).
...
// ─── OpenAI Chat Completions com Structured Outputs strict ─────
...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    tipo_documento: {
      type: 'string',
      enum: ['NF_PDF', 'NFSE_PDF', 'DANFE_PDF', 'NF_XML', 'COMPROVANTE', 'RECIBO_FISCAL', 'ORCAMENTO', 'PROPOSTA', 'EXTRATO', 'DOCUMENTO_ADMINISTRATIVO', 'NAO_IDENTIFICADO'],
    },
    ...
    status_revisao: {
      type: 'string',
      enum: ['AGUARDANDO_REVISAO', 'REVISAR', 'BLOQUEADO', 'PRE_APROVADO'],
    },
  ...
  required: [
    'tipo_documento',
    'documento_fiscal_valido',
    'criar_solicitacao_financeira',
    'numero_nota',
    'serie',
    'chave_acesso',
    'data_emissao',
...
// ─── Prompt de sistema (instruções ativas) ─────────────────────
...
async function carregarContexto(base44, intake) {
  ...
  // Rubricas ativas (não enviar o banco inteiro — só campos essenciais)
  ...
  // Metas ativas (não enviar descrições longas)
  ...
  // Compras candidatas: NFs aprovadas/coordenador com mesmo CNPJ/CPF ou recentes (180 dias)
  const cnpj = onlyDigits(intake?.fornecedor_cpf_cnpj || intake?.nf_emitente_cpf_cnpj || '');
  const periodo = ((d) => (d ? new Date(Date.now() - Number(d) * 86400000).toISOString() : null))(180);
  const filtroCompra = cnpj ? { fornecedor_cnpj: cnpj } : {};
  let compras = [];
  try {
    compras = await svc.entities.PurchaseRequest.filter(filtroCompra, '-created_date', 80);
  } catch (e) {
    console.warn('[lerNF] Filtro PurchaseRequest falhou, seguindo sem compras candidatas:', e.message);
  }
  if (!compras || !compras.length) {
    try {
      compras = await svc.entities.PurchaseRequest.filter({}, '-created_date', 80);
    } catch {}
  }
  ...
  // Documentos possivelmente duplicados (mesmo fornecedor + mesmo número, ou mesmo nome de arquivo)
  let duplicados = [];
  ...
  return {
    ...
    duplicadosCompras: comprasCandidatas.slice(40, 80),
...
function validarResultado(ia, ctx) {
  ...
  // Valor total zero em documento fiscal válido
  ...
  // Tomador — valida contra o tomador esperado
  ...
  // Consistência rubrica <-> meta (quando o usuário manual vinculou)
  ...
  // Recalcular status_revisao final conforme score resultante
...
// ─── Tipos de conteúdo multimodal para a IA ───────────────────
...
Deno.serve(async (req) => {
  try {
    ...
    const { intake_id, file_url, file_name, mime_type, xml_url, contexto = {} } = body;
    ...
    if (intake_id) {
      try {
        ...
        return Response.json({ ok: false, error: 'Intake não encontrado: ' + e.message }, { status: 404 });
      }
      if (!intake) return Response.json({ ok: false, error: 'Intake não encontrado' }, { status: 404 });
    ...
    if (!fileUrl) {
      return Response.json({ ok: false, error: 'file_url ou intake_id é obrigatório.' }, { status: 400 });
    }

    // Carrega contexto uma única vez
    const ctx = await carregarContexto(base44, intake);

    // 1) Parse XML determinístico (fonte primária quando houver)
    let dadosXMLRaw = null;
    if (xmlUrl) {
      try {
        const xmlText = await fetchText(xmlUrl).catch(() => null);
        if (xmlText && xmlText.includes('<')) {
          dadosXMLRaw = parseXmlRaw(xmlText);
        }
      } catch (e) {
        console.warn('[lerNF] XML parse falhou:', e.message);
      }
    }
    // Coleta xml do intake contextual caso tenham sido passados
    if (!dadosXMLRaw && contexto.xml_text && contexto.xml_text.includes('<')) {
      dadosXMLRaw = parseXmlRaw(contexto.xml_text);
    }

    // 2) Identifica tipo de arquivo principal
    const tipoArq = detectFileType(fileName, mimeType);

    // 3) Constrói partes do conteúdo a enviar ao modelo
    const userParts = [];

    if (dadosXMLRaw) {
      userParts.push({
        type: 'text',
        text: `XML fiscal pré-extraído (fonte primária, priorizar sobre qualquer PDF): ${JSON.stringify(dadosXMLRaw)}`,
      });
    }
    if (intake) {
    ...
    let ia;
    // 3.1) Puro XML — sem chamada à IA (já é determinístico)
    if (tipoArq === 'xml' || (!fileUrl && dadosXMLRaw)) {
      ...
      // 3.2) Download + (upload para OpenAI ou uso via base64)
      let bytes;
      try {
        bytes = await resolverDownload(base44, fileUrl);
      } catch (e) {
        // Sem arquivo fisicamente disponível — ainda tenta com XML + contexto
        if (dadosXMLRaw) {
          ia = laudoSomenteXML(dadosXMLRaw, ctx, fileName);
        } else {
          return Response.json({ ok: false, error: 'Falha no download: ' + e.message }, { status: 502 });
        }
      }
      if (bytes && !ia) {
        if (bytes.length > 25 * 1024 * 1024) {
          return Response.json({ ok: false, error: 'Arquivo excede 25MB.' }, { status: 413 });
        }
        if (tipoArq === 'pdf') {
          try {
            ...
            return Response.json({ ok: false, error: 'Falha upload Files API: ' + e.message }, { status: 502 });
          }
        } else if (tipoArq === 'image') {
          const dataUrl = bytesToDataURL(bytes, mimeType || 'image/jpeg');
          userParts.push({ type: 'image_url', image_url: { url: dataUrl, detail: 'high' } });
        } else {
          // Documento genérico (docx/txt/etc) — envia como texto curado
          try {
            const texto = new TextDecoder('utf-8').decode(bytes).slice(0, 30000);
            userParts.push({
              type: 'text',
              text: `Conteúdo textual extraído do arquivo: ${texto}`,
            });
          } catch {
            return Response.json({ ok: false, error: 'Tipo de arquivo não suportado para análise.' }, { status: 415 });
          }
        }
        ...
        try {
          ia = await invokeGPTStructured({
            systemPrompt,
            userParts,
            schema: RESPONSE_SCHEMA,
          });
        } catch (e) {
          // Se IA falhou mas tenho XML determinístico, follow fallback
          if (dadosXMLRaw) {
            ...
            return Response.json({ ok: false, error: 'Falha na análise GPT: ' + e.message }, { status: 502 });
          }
        }
      }
    } else {
      // Sem arquivo para IA — se XML existe, usa laudo só XML
      if (dadosXMLRaw) {
        ia = laudoSomenteXML(dadosXMLRaw, ctx, fileName);
      } else {
        return Response.json({ ok: false, error: 'Não há arquivo nem XML válidos para análise.' }, { status: 400 });
      }
    }

    // 4) Validação determinística pós-IA
    const contextoFinal = {
      ...ctx,
      dadosXMLRaw,
    ...
    // 5) Log de auditoria (sem chave da API)
    try {
      ...
      console.warn('[lerNF] Falha ao gravar AIUsageLog:', e.message);
    }
...
function laudoSomenteXML(xml, ctx, fileName) {
  const tipoDocumento = (fileName || '').toUpperCase().includes('NFSE') ? 'NFSE_PDF' : 'NF_XML';
  const score = 7;
  const evidencias = {
    numero_nota: xml.nf_numero || null,
    data_emissao: xml.nf_data_emissao || null,
    valor_total: xml.nf_valor_total ? `R$ ${xml.nf_valor_total.toFixed(2)}` : null,
    fornecedor_nome: xml.nf_emitente_nome || null,
    tomador_cnpj: xml.tomador_cnpj || null,
    rubrica_nome: null,
    meta_nome: null,
    centro_custo: null,
  };
  return {
    ...
    documento_fiscal_valido: !!(xml.nf_numero && (xml.nf_emitente_cpf_cnpj || xml.nf_emitente_nome)),
    criar_solicitacao_financeira: !!(xml.nf_numero && xml.nf_valor_total > 0),
    numero_nota: xml.nf_numero || null,
    serie: xml.nf_serie || null,
    chave_acesso: xml.nf_chave_acesso || null,
    data_emissao: xml.nf_data_emissao || null,
    data_competencia: xml.competencia || null,
    data_vencimento: null,
    fornecedor_nome: xml.nf_emitente_nome || null,
    fornecedor_cnpj: onlyDigits(xml.nf_emitente_cpf_cnpj).length === 14 ? xml.nf_emitente_cpf_cnpj : null,
    fornecedor_cpf: onlyDigits(xml.nf_emitente_cpf_cnpj).length === 11 ? xml.nf_emitente_cpf_cnpj : null,
    fornecedor_inscricao_municipal: null,
    tomador_nome: xml.tomador_nome || null,
    tomador_cnpj: xml.tomador_cnpj || null,
    tomador_valido: (onlyDigits(xml.tomador_cnpj) === TOMADOR_BENEFAVIADO_CNPJ),
    descricao_original: xml.descricao_servico || null,
    descricao_normalizada: null,
    valor_servicos: xml.nf_valor_total || null,
    valor_produtos: null,
    valor_total: xml.nf_valor_total || null,
    valor_liquido: xml.nf_valor_liquido || null,
    iss: xml.iss || null,
    inss: xml.inss || null,
    irrf: xml.irrf || null,
    csll: xml.csll || null,
    cofins: xml.cofins || null,
    pis: xml.pis || null,
    outras_retencoes: null,
    municipio_emissao: xml.municipio || null,
    codigo_servico_municipal: xml.codigo_servico || null,
    cnae: xml.cnae || null,
