...
function normalizeSheet(sheetName: string, matrix: any[][], rowOffset = 0) {
  ...
  const headers = (matrix[0] || []).map((value: any, index: number) => normalizeHeader(value, index));
  ...
  return rows
    .map((row, index) => {
      ...
      const firstText =
        (row || []).find((cell: any) => String(cell || '').trim() !== '') || '';

      const structured = mapStructuredFields(values, firstText, index + 2 + rowOffset, sheetName);

      return {
        row_index: index + 2 + rowOffset,
    ...
    .filter((item) => {
      const hasAnyValue =
        ...
        item.descricao ||
        item.data ||
        Object.values(item.values || {}).some((v) => String(v || '').trim() !== '');
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    workbook.SheetNames.forEach((sheetName) => {
      ...
      runningOffset += normalizedItems.length + 5;
    });
