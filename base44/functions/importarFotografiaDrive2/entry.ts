...
const ROOT_FOLDER_ID = '1rnpwK5eEY0bPFLbmyqfzzzyxbw9Zm3oh';
...
function extrairMuseu(nomePasta: string){
  ...
  if(t.includes('mhab')||t.includes('museu da habitação')) return 'MHAB';
    ...
    if(modo === 'processar'){
      ...
      for(const img of imgs){
        try {
          ...
          const nomePad = `GALERIA2_${museu.replace(/\s+/g,'_')}_${String(mesNum||'00').padStart(2,'0')}_${ano}_${img.name.replace(/\s+/g,'_').slice(0,60)}`.replace(/[^a-zA-Z0-9_.-]/g,'_');
          ...
          await base44.asServiceRole.entities.ReportPhoto.create({
            ...
            fonte_ia: 'drive_sync',
            contexto_ia: JSON.stringify({ pasta_origem: nomePasta, caminho: (img._path||[]).join('/'), origem: 'Importação Fotografia Drive 2' }),
          });
          criadas++;
        } catch(error: any){
          erros++;
