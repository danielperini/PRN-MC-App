...
const MAX_DOWNLOADS = 25; // downloads por chamada
...
Deno.serve(async (req) => {
  try {
    ...
    const folderId = String(body.folder_id || '1gMPRXyamu9YANVFg6Xf7VtWoOoF-3CbQ').trim();
    const museuFiltro = body.museu ? normalizeMuseu(body.museu) : null;
    ...
    const imagens = museuFiltro
      ? imagensEncontradas.filter((img: any) => normalizeMuseu([...(img._path || []), img.name].join(' ')) === museuFiltro)
      : imagensEncontradas;
    ...
    // Classificar todas as imagens e filtrar apenas as que precisam de download
    const todasClassificadas = imagens.map(img => classificarImagem(img, reports, fotosExistentes, attachments, existentePorDrive));
    const paraImportar = todasClassificadas.filter(r => !r.ja_importada || r.precisa_reparar);
    const bloco = paraImportar.slice(offset, offset + MAX_DOWNLOADS);

    let criadas = 0; let reparadas = 0; let erros = 0;
    const falhas: any[] = [];
    const porMuseu: Record<string, number> = {};

    for(const foto of bloco){
      try {
        const img = imagens.find(i => i.id === foto.drive_file_id);
        if(!img) throw new Error('Arquivo não localizado no Drive.');
        ...
        if(foto.museu) porMuseu[foto.museu] = (porMuseu[foto.museu] || 0) + 1;
      } catch(error: any){
        erros++;
    ...
    const totalNovas = paraImportar.filter(r => !r.precisa_reparar).length;
    const totalReparar = paraImportar.filter(r => r.precisa_reparar).length;
    const totalJaImportadas = todasClassificadas.length - paraImportar.length;
    const nextOffset = offset + MAX_DOWNLOADS;
    const hasMore = nextOffset < paraImportar.length;

    if(!hasMore && (criadas + reparadas) > 0){
      await base44.asServiceRole.entities.AuditLog.create({
        action: 'CREATE', entity_type: 'REPORT_PHOTO', entity_id: 'batch_varredura_pdf',
        actor_email: user.email, actor_name: user.full_name || user.email,
        details: `Varredura pré-PDF: ${criadas} criadas, ${reparadas} reparadas, ${erros} erros. Pasta: ${folderId}, Museu: ${museuFiltro || 'todos'}`,
      }).catch(() => {});
    }
