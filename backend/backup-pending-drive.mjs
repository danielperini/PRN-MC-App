...
async function main() {
  ...
  const rows=(await pool.query("SELECT * FROM purchase_requests WHERE COALESCE(nota_fiscal_url,nota_fiscal_pdf_url,arquivo_url,'')<>'' AND COALESCE(drive_file_id,'')='' AND COALESCE(drive_file_url,'')='' AND COALESCE(drive_backup_nf_pdf_link,'')='' AND COALESCE(drive_backup_status,'')<>'CONCLUIDO'")).rows;
  let backed=0, skipped=0, failed=0;
  for(const p of rows) { try { const date=dateOf(p.nf_data_emissao || p.data_emissao); const url=sourceOf(p); const file=localOf(url); if(!date || !file) { skipped++; continue; } const parent=await folder(date); const name=nameOf(p,url); const found=await drive.files.list({q:`'${parent}' in parents and name='${escapeDrive(name)}' and trashed=false`,fields:'files(id,webViewLink)',pageSize:1,supportsAllDrives:true,includeItemsFromAllDrives:true}); const remote=found.data.files?.[0] || (await drive.files.create({requestBody:{name,parents:[parent]},media:{mimeType:'application/pdf',body:fs.createReadStream(file)},fields:'id,webViewLink',supportsAllDrives:true})).data; const link=remote.webViewLink || `https://drive.google.com/file/d/${remote.id}/view`; await pool.query('UPDATE purchase_requests SET drive_file_id=$1, drive_file_url=$2, drive_backup_nf_pdf_link=$2, drive_backup_status=$3 WHERE id=$4',[remote.id,link,'CONCLUIDO',p.id]); backed++; } catch(error) { failed++; console.error('BACKUP_PENDING_FAILED',p.id,error.message); } }
