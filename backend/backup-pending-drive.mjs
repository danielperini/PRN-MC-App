...
async function main() {
  ...
  const rows=(await pool.query("SELECT * FROM purchase_requests WHERE (COALESCE(nota_fiscal_url,'')<>'' OR COALESCE(nota_fiscal_pdf_url,'')<>'' OR COALESCE(arquivo_url,'')<>'') AND COALESCE(drive_file_id,'')='' AND COALESCE(drive_file_url,'')='' AND COALESCE(drive_backup_nf_pdf_link,'')='' AND COALESCE(drive_backup_status,'')<>'CONCLUIDO'")).rows;
