...
import { ArrowLeft, Save, Send, Plus, Trash2, ShieldCheck, FileDown } from 'lucide-react';
...
function ReportEditorInner() {
  ...
  useEffect(() => {
    ...
    if (!reportId) setFormData(EMPTY_FORM);
  }, [reportId]);
  ...
  useEffect(() => {
    ...
    if (activitiesFromDB.length > 0) {
      merged = activitiesFromDB.map((a) => ({
        ...a,
        nome: a.nome || a.titulo || '',
        quantidade_ocorrencias:
          a.quantidade_ocorrencias ??
          a.quantas_repeticoes ??
          1,
        quantidade_produtos_gerados:
          a.quantidade_produtos_gerados ??
          a.quantidade_produtos ??
          0,
      }));
    }

    for (const emb of embedded) {
      if (emb.id && entityById.has(emb.id)) continue;
      ...
      if (!exists) {
        merged.push({
          ...emb,
          quantidade_ocorrencias:
            emb.quantidade_ocorrencias ??
            emb.quantas_repeticoes ??
            1,
          quantidade_produtos_gerados:
            emb.quantidade_produtos_gerados ??
            emb.quantidade_produtos ??
            0,
        });
      }
    }
  ...
  const gerarNumeroProtocolo = async (mes, ano) => {
    ...
    const mesAbrev =
      MESES_ABREV[mes] || String(mes || 'SEM').substring(0, 3).toUpperCase();
    ...
    for (const atv of atividades || []) {
      ...
      const payload = {
        ...
        quantidade_produtos_gerados:
          ...
          0,
        equipe_responsavel:
          Array.isArray(rest.museu_lista) && rest.museu_lista.length
        ...
        observacoes:
          [
            ...
            .join(' | ') || '',
        resultado_alcancado:
          rest.produto_realizado || rest.resultado_alcancado || '',
        meta_quantitativa:
          rest.total_atividades != null
            ? String(rest.total_atividades)
            : rest.meta_quantitativa || '',
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      ...
      if (savedId) await syncActivities(savedId, atividades);
    ...
    onSuccess: (saved) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['activities', savedId] });
  ...
  const handleSaveDraft = () => {
    ...
    toast.promise(promise, {
      ...
      success: `Rascunho salvo! ${formData.atividades?.length || 0} atividade(s) gravada(s).`,
      error: (err) => `Erro ao salvar: ${err?.message || 'Tente novamente.'}`,
    });
  };
  ...
  const submitMutation = useMutation({
    mutationFn: async () => {
      if (!declaracaoAceita) {
        throw new Error('Aceite a declaração de responsabilidade antes de enviar.');
      }
      if (!formData.mes_referencia) {
        throw new Error('Selecione o mês de referência antes de enviar.');
      }
      if (!formData.author_name) {
        throw new Error('Informe o nome do profissional antes de enviar.');
      }
      if (!formData.museu) {
        throw new Error('Selecione o museu antes de enviar.');
      }
  ...
  const handleConfirmSubmit = () => {
    ...
    toast.promise(promise, {
      loading: 'Enviando relatório para revisão...',
      success: 'Relatório enviado! O coordenador será notificado.',
      error: (err) => err?.message || 'Não foi possível enviar. Tente novamente.',
    });
  };
    ...
    onSuccess: (_, { action }) => {
      ...
      const msgs = {
        start_review: {
          ...
          desc: '✓ O relatório foi movido para revisão.',
        },
        return: {
          title: 'Relatório devolvido',
          desc: '✓ Notificação enviada ao profissional.',
        },
        approve: {
          title: 'Relatório aprovado',
          desc: '✓ Status atualizado com sucesso.',
        },
        archive: {
          title: 'Arquivado',
          desc: '✓ Relatório movido para arquivo.',
        },
        reopen: {
          title: 'Reabertura concluída',
          desc: '✓ Relatório reaberto como rascunho.',
        },
      };
  ...
  const updateOp = (i, field, value) =>
    setFormData((prev) => ({
      ...prev,
      oportunidades: prev.oportunidades.map((o, idx) =>
  ...
  const handleTabChange = (newTab) => {
    if (canEdit && reportId) saveMutation.mutate(formData);
        ...
        <div className="flex items-center justify-between mb-10 flex-wrap gap-3">
          ...
          {reportId && (
            <Button
              variant="outline"
              size="sm"
              onClick={() => setShowConsolidatedExport(true)}
            >
        ...
        <Tabs value={currentTab} onValueChange={handleTabChange} className="w-full">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <Field label="Mês de Referência">
                    <Select
                      ...
                      onValueChange={(v) => set('mes_referencia', v)}
                      disabled={!canEdit}
                    >
                      <SelectTrigger>
                  ...
                  <Field label="Ano">
                    <Input
                      ...
                      onChange={(e) =>
                        set('ano', parseInt(e.target.value, 10) || 2026)
                      }
                      disabled={!canEdit}
                    />
                  </Field>
                ...
                {(formData.oportunidades || []).map((op, i) => (
                  <div key={i} className="p-4 border rounded-xl space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <Select
                        value={op.categoria || ''}
                        onValueChange={(v) => updateOp(i, 'categoria', v)}
                        disabled={!canEdit}
                      >
                        <SelectTrigger>
                      ...
                      <Select
                        value={op.impacto || ''}
                        onValueChange={(v) => updateOp(i, 'impacto', v)}
                        disabled={!canEdit}
                      >
                        <SelectTrigger>
              ...
              {canEdit && (
                <div className="p-4 border border-gray-200 rounded-xl bg-gray-50">
                  <div className="flex items-start gap-3">
                    ...
                    >
                      Declaro que as informações registradas neste relatório são
                      verdadeiras, completas e de minha inteira responsabilidade.
                    </label>
                  </div>
                </div>
              )}
          ...
          <TabsContent value="anexos" className="space-y-6">
            {reportId ? (
              ...
              <p className="text-sm text-gray-400 text-center py-8">
                Salve o relatório para anexar arquivos
              </p>
            )}
          </TabsContent>
          ...
          <TabsContent value="comentarios" className="space-y-6">
            {reportId ? (
              ...
              <p className="text-sm text-gray-400 text-center py-8">
                Salve o relatório para adicionar comentários
              </p>
            )}
          </TabsContent>
          ...
          <TabsContent value="historico" className="space-y-6">
            {reportId ? (
              ...
              <p className="text-sm text-gray-400 text-center py-8">
                Salve o relatório para visualizar histórico
              </p>
            )}
          </TabsContent>
        </Tabs>
          ...
          <AlertDialogContent className="max-w-lg">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="mt-3 text-sm text-gray-700">
                Ao prosseguir, você atesta que todas as informações deste relatório
                são verdadeiras e completas.
              </AlertDialogDescription>
            </AlertDialogHeader>
