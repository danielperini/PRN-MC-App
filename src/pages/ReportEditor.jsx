...
function ReportEditorInner() {
  ...
  const set = (key, value) => {
    setFormData((prev) => {
      ...
      if (key === 'atividades' && Array.isArray(value)) next.atividades = value.map((item) => ({ ...item }));
      if (key === 'oportunidades' && Array.isArray(value)) next.oportunidades = value.map((item) => ({ ...item }));
      if (key === 'momentos' && Array.isArray(value)) next.momentos = value.map((item) => ({ ...item }));
      if (key === 'depoimentos' && Array.isArray(value)) next.depoimentos = value.map((item) => ({ ...item }));
      return next;
    });
  };
  ...
  useEffect(() => {
    ...
    for (const emb of embedded) {
      ...
      const exists = merged.some(a => a.nome === emb.nome && a.data_inicio === emb.data_inicio);
      if (!exists) merged.push({ ...emb });
    }
    ...
    setFormData({
      ...
      ...reportData,
      atividades: merged,
      oportunidades: Array.isArray(reportData.oportunidades) ? reportData.oportunidades.map(i => ({ ...i })) : [],
      momentos: Array.isArray(reportData.momentos) ? reportData.momentos.map(i => ({ ...i })) : [],
      depoimentos: Array.isArray(reportData.depoimentos) ? reportData.depoimentos.map(i => ({ ...i })) : []
    });
  ...
  const submitMutation = useMutation({
    mutationFn: async () => {
      if (!declaracaoAceita) throw new Error('Aceite a declaração de responsabilidade antes de enviar.');
      if (!formData.mes_referencia) throw new Error('Selecione o mês de referência antes de enviar.');
      if (!formData.author_name) throw new Error('Informe o nome do profissional antes de enviar.');
      if (!formData.museu) throw new Error('Selecione o museu antes de enviar.');
          ...
          <TabsContent value="atividades">
            <div className="space-y-6">
              <AtividadesSection
                ...
                tiposAcaoOptions={[
                  'Oficina', 'Visita mediada', 'Ação educativa', 'Comunicação',
                  'Reunião', 'Produção', 'Articulação', 'Formação', 'Outro'
                ]}
              />
            </div>
          </TabsContent>
