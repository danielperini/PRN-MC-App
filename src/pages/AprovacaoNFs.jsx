...
function NFCard({ intake, onAprovar, onRejeitar, processando, canSeeAll }) {
  ...
  return <div className={`bg-white rounded-2xl border shadow-sm overflow-hidden ${xml ? 'border-gray-200' : 'border-orange-200'}`}>
    ...
    <div className="p-4 space-y-3">
      <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-sm font-bold text-slate-900 truncate" title={fileName}>{fileName}</p><p className="text-xs text-gray-400 truncate mt-0.5">{fornecedor}</p>{canSeeAll && solicitante && <p className="text-xs text-blue-600 font-medium mt-0.5">👤 {solicitante}</p>}<div className="flex gap-2 mt-2 flex-wrap"><StatusXML intake={intake} />{semXmlFlag && <span className="inline-flex items-center gap-1 text-[10px] font-semibold bg-orange-100 text-orange-700 rounded-full px-2 py-1"><AlertTriangle className="w-3 h-3" />Sem XML</span>}{numeroNF && <span className="text-[10px] bg-gray-100 rounded-full px-2 py-1">NF {numeroNF}</span>}{score != null && <span className="text-[10px] bg-blue-50 text-blue-700 rounded-full px-2 py-1">IA {Number(score) <= 1 ? Math.round(Number(score) * 100) : Math.round(Number(score))}%</span>}</div></div><div className="text-right shrink-0"><p className="text-lg font-bold">{fmtBRL(valor)}</p>{centro && <span className="text-[10px] bg-gray-100 rounded-full px-2 py-1">{centro}</span>}</div></div>
