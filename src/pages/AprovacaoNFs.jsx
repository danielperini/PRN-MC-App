...
import { FileText, CheckCircle, XCircle, ExternalLink, Search, X, FileCode2, AlertCircle, Loader2, RefreshCw, Filter, ShieldCheck, AlertTriangle } from 'lucide-react';
...
function NFCard({ intake, onAprovar, onRejeitar, processando, canSeeAll }) {
  ...
  const semXmlFlag = d.xml_obrigatorio_pendente || intake.xml_obrigatorio_pendente || d.enviado_sem_xml || intake.enviado_sem_xml;
  const xmlPendenteSince = d.xml_pendente_desde || intake.xml_pendente_desde;
  const diasSolicitado = xmlPendenteSince ? Math.floor((Date.now() - new Date(xmlPendenteSince).getTime()) / 86400000) : null;
  ...
  return <div className={`bg-white rounded-2xl border shadow-sm overflow-hidden ${xml ? 'border-gray-200' : 'border-orange-200'}`}>
    ...
    <div className="p-4 space-y-3">
      <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-sm font-bold text-slate-900 truncate" title={fileName}>{fileName}</p><p className="text-xs text-gray-400 truncate mt-0.5">{fornecedor}</p>{canSeeAll && solicitante && <p className="text-xs text-blue-600 font-medium mt-0.5">👤 {solicitante}</p>}<div className="flex gap-2 mt-2 flex-wrap"><StatusXML intake={intake} />{semXmlFlag && (
        <span className="inline-flex items-center gap-1 text-[10px] font-semibold bg-orange-100 text-orange-700 rounded-full px-2 py-1">
          <AlertTriangle className="w-3 h-3" />
          Sem XML{diasSolicitado !== null ? ` · Solicitado há ${diasSolicitado === 0 ? 'hoje' : `${diasSolicitado}d`}` : ''}
        </span>
      )}{numeroNF && <span className="text-[10px] bg-gray-100 rounded-full px-2 py-1">NF {numeroNF}</span>}{score != null && <span className="text-[10px] bg-blue-50 text-blue-700 rounded-full px-2 py-1">IA {Number(score) <= 1 ? Math.round(Number(score) * 100) : Math.round(Number(score))}%</span>}</div></div><div className="text-right shrink-0"><p className="text-lg font-bold">{fmtBRL(valor)}</p>{centro && <span className="text-[10px] bg-gray-100 rounded-full px-2 py-1">{centro}</span>}</div></div>
