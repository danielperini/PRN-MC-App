...
import { agruparMovimentacoesPorMes, resumirGruposMensais } from '@/utils/movimentacoesMensais';
          ...
          <tbody className="divide-y divide-gray-100">
            {exibidos.map((l, i) => (
              <tr key={i} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3"><TipoChip tipo={l.tipo} /></td>
...
function MovimentacoesInner() {
  ...
  async function handleSincronizarDocs() {
    ...
    try {
      ...
      if (novos_no_drive === 0) toast.success('Todos os documentos já estão importados.');
      else toast.success(`${criados} documento${criados !== 1 ? 's' : ''} importado${criados !== 1 ? 's' : ''}${restantes > 0 ? ` · ${restantes} restante${restantes !== 1 ? 's' : ''} — clique novamente` : ''}${erros > 0 ? ` · ${erros} erro${erros !== 1 ? 's' : ''}` : ''}`, { duration: restantes > 0 ? 8000 : 4000 });
    } catch (e) { toast.error('Erro: ' + (e?.message || String(e)), { duration: 6000 }); }
  ...
  async function handleSincronizar() {
    ...
    try {
      ...
      if (pdfs_encontrados === 0) toast.info('Nenhum PDF encontrado nas pastas mensais de extratos do Drive.');
      else if (novos_criados === 0 && restantes === 0) toast.success('Todos os extratos já estão importados.');
      else toast.success(`${novos_criados} extrato${novos_criados !== 1 ? 's' : ''} importado${novos_criados !== 1 ? 's' : ''}${restantes > 0 ? ` · Clique novamente para processar ${restantes} restante${restantes !== 1 ? 's' : ''}` : ''}${erros > 0 ? ` · ${erros} erro${erros !== 1 ? 's' : ''}` : ''}`, { duration: restantes > 0 ? 8000 : 4000 });
      await refetch();
    } catch (e) { toast.error('Erro ao sincronizar: ' + (e?.message || String(e)), { duration: 6000 }); }
  ...
  return (
    <div className="min-h-screen bg-gray-50/50"><div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
      ...
      {!isLoading && movimentacoes.length > 0 && <><div className="grid grid-cols-1 sm:grid-cols-3 gap-3"><div className="rounded-2xl border border-green-200 bg-white p-5 flex items-center gap-4 shadow-sm"><div className="w-10 h-10 rounded-xl bg-green-100 flex items-center justify-center shrink-0"><ArrowUpRight className="w-5 h-5 text-green-600" /></div><div><p className="text-[11px] text-green-500 uppercase font-bold tracking-wide">Total créditos</p><p className="text-xl font-bold text-green-700">{fmtBRL(totaisGerais.creditos)}</p><p className="text-[10px] text-gray-400 mt-0.5">Período completo · {grupos.length} mese{grupos.length !== 1 ? 's' : ''}</p></div></div><div className="rounded-2xl border border-red-200 bg-white p-5 flex items-center gap-4 shadow-sm"><div className="w-10 h-10 rounded-xl bg-red-100 flex items-center justify-center shrink-0"><ArrowDownLeft className="w-5 h-5 text-red-500" /></div><div><p className="text-[11px] text-red-400 uppercase font-bold tracking-wide">Total débitos</p><p className="text-xl font-bold text-red-600">{fmtBRL(totaisGerais.debitos)}</p><p className="text-[10px] text-gray-400 mt-0.5">Período completo · {movimentacoes.length} extrato{movimentacoes.length !== 1 ? 's' : ''}</p></div></div><div className="rounded-2xl border border-blue-200 bg-white p-5 flex items-center gap-4 shadow-sm"><div className="w-10 h-10 rounded-xl bg-blue-100 flex items-center justify-center shrink-0"><TrendingUp className="w-5 h-5 text-blue-600" /></div><div><p className="text-[11px] text-blue-500 uppercase font-bold tracking-wide">Total rendimentos</p><p className="text-xl font-bold text-blue-700">{fmtBRL(totaisGerais.rendimento)}</p><p className="text-[10px] text-gray-400 mt-0.5">Aplicações e rendimentos</p></div></div></div><FluxoCaixaMensal /><div><p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3 flex items-center gap-1.5"><CalendarDays className="w-3.5 h-3.5" /> Selecione o mês</p><div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1">{grupos.map(({ key, registros }) => <CardMes key={key} registros={registros} selecionado={mesSelecionado === key} onSelecionar={() => setMesSelecionado(key)} />)}</div></div><div className="relative"><Search className="absolute left-3.5 top-3 w-4 h-4 text-gray-400" /><input type="text" value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar lançamento, banco, descrição…" className="w-full pl-10 pr-10 py-2.5 text-sm rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-slate-800 bg-white shadow-sm" />{busca && <button onClick={() => setBusca('')} className="absolute right-3.5 top-3 text-gray-400 hover:text-black"><X className="w-4 h-4" /></button>}</div><div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">{registrosMes.length > 0 ? <DetalheMes registros={registrosMes} busca={busca} /> : <div className="py-16 text-center"><CalendarDays className="w-10 h-10 text-gray-300 mx-auto mb-3" /><p className="text-gray-400">Selecione um mês acima para ver os detalhes</p></div>}</div></>}
    </div></div>
  );
}
