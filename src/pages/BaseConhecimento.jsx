...
import { b// 🔥 VERSÃO FINAL SEM ENTITY (GOOGLE SHEETS DIRETO + ESPELHO COMPLETO)
...
function MirrorItemCard({ item, index }) {
  const titulo = item?.titulo || item?.first_text || `Linha ${index + 1}`;
  ...
  return (
    <div className="border rounded-lg p-4 space-y-2">
      ...
      <div className="text-xs text-gray-600 flex gap-2 flex-wrap">
        {data && <span>{data}</span>}
        {museu && <span>· {museu}</span>}
        {equipe && <span>· {equipe}</span>}
        {classificacao && <span>· {classificacao}</span>}
      </div>
...
function BaseConhecimentoInner() {
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6">
        <Input
          placeholder="Buscar na planilha espelhada, incluindo meses anteriores"
          value={busca}
      ...
      <div className="space-y-3 mb-8">
        {itensFiltrados.map((item, index) => (
          <MirrorItemCard
            key={`${item?.row_index || index}-${item?.titulo || item?.first_text || index}`}
            item={item}
            index={index}
          />
        ))}
        ...
        {itensFiltrados.length === 0 ? (
          <div className="border rounded-lg p-6 text-sm text-gray-500">
            Nenhum item encontrado no espelho da planilha.
          </div>
        ) : null}
      </div>
