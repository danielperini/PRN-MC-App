...
function ComprasInner() {
  ...
  // ── Recálculo geral via backend — fonte da verdade ────────────────────────
  async function recalcularTodasRubricas() {
    if (!window.confirm('Recalcular rubricas com base oficial do 3º Aditivo?\n\nEssa operação usa o backend como fonte da verdade.')) return;
    ...
    try {
      ...
      if (!result?.success) {
        throw new Error(result?.error || 'Erro no recálculo');
      }
