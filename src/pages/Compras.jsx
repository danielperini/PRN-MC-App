...
function getChaveFiscal(p) {
  ...
  if (p?.nota_fiscal_url) return `url:${String(p.nota_fiscal_url).trim()}`;
  if (p?.file_url) return `file:${String(p.file_url).trim()}`;
  if (p?.intake_id) return `intake:${String(p.intake_id).trim()}`;
...
function getPurchaseXmlUrl(purchase, attachmentByPurchaseId = {}) {
  return (
    purchase?.xml_url ||
    purchase?.nf_xml_url ||
    purchase?.nota_fiscal_xml_url ||
    attachmentByPurchaseId?.[purchase?.id]?.xml_url ||
    ''
  );
}
...
function isRubricaAtiva(r) {
  return (
    r?.ativo !== false &&
    normalizeStatus(r?.status) !== 'INATIVA_DUPLICADA' &&
    normalizeStatus(r?.status) !== 'INATIVA_FORA_BASE_OFICIAL'
  );
}
...
async function carregarRubricas() {
  try {
    ...
    if (Array.isArray(viaFunction) && viaFunction.length > 0) {
      return viaFunction.filter(isRubricaAtiva);
    }
  } catch (error) {
  ...
  try {
    const diretas = await base44.entities.Rubrica.list('codigo', 500);
    if (Array.isArray(diretas)) return diretas.filter(isRubricaAtiva);
  } catch (error) {
      ...
      <table className="w-full table-fixed border-collapse text-sm">
        ...
        <thead>
          <tr className="border-b border-gray-200 bg-gray-50 text-left">
            ...
            <th className="px-3 py-3 text-center font-medium text-gray-600">Arquivos</th>
        ...
        <tbody>
          {purchases.map((p, i) => {
            ...
            const xmlUrl = getPurchaseXmlUrl(p, attachmentByPurchaseId);
              ...
              >
                ...
                <td className="px-3 py-2.5 align-top text-center">
                  <div className="flex flex-col items-center gap-1">
                    ...
                    {!fileUrl && !xmlUrl && <span className="text-xs text-amber-600">Solicitar arquivos</span>}
                  </div>
                </td>
                    ...
                    {isCoordenador && (
                      <button
                        ...
                        onClick={async (e) => {
                          e.preventDefault();
                          e.stopPropagation();
                          if (window.confirm('Tem certeza que deseja deletar esta solicitação?')) await onDelete(p.id);
                        }}
...
function ComprasInner() {
  ...
  async function recalcularTodasRubricas() {
    if (
      !window.confirm(
        'Restaurar e recalcular rubricas com base oficial do 3º Aditivo?\n\nEssa operação usa o backend como fonte da verdade e pode ser repetida com segurança.'
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (!result?.success) {
        throw new Error(result?.error || 'Erro no recálculo das rubricas.');
      }

      await refreshFinanceiroCompleto();
      smartToast.success('Rubricas restauradas e recalculadas com base oficial.');
    } catch (error) {
      console.error('Erro no recálculo:', error);
      smartToast.error('Erro ao recalcular', error.message);
    } finally {
        ...
        {isCoordenador && (
          <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
              ...
              <p className="text-xs text-gray-400">APROVADO = UTILIZADO</p>
            </div>
            ...
            <div className="mb-4 flex flex-wrap gap-3">
              ...
              <Select value={filters.rubrica_id} onValueChange={(v) => setFilters((f) => ({ ...f, rubrica_id: v }))}>
                ...
                <SelectContent>
                  ...
                  {(rubricas || []).filter(isRubricaAtiva).map((r) => (
                    <SelectItem key={r.id} value={r.id}>
                      {r.rubrica || r.nome || r.item_rubrica || r.descricao}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div className="mb-3 flex items-center justify-between">
              ...
              {isCoordenador && (
                ...
                >
                  ...
                  {recalculando ? 'Recalculando...' : 'Restaurar/Recalcular Rubricas'}
                </Button>
              )}
            </div>
