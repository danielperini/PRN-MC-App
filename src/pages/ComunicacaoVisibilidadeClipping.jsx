...
import { ExternalLink, Search, Newspaper, TrendingUp, Share2, Globe2, CalendarDays, Sparkles, FolderOpen, Image, Megaphone, Hash } from 'lucide-react';
import { Input } from '@/components/ui/input';
import { Badge } from '@/components/ui/badge';
import { Card, CardContent } from '@/components/ui/card';

const HEAD_KEYWORDS = [
  'Museus Centro', 'Viaduto das Artes', 'Museu Histórico Abílio Barreto', 'Abílio Barreto', 'MHAB',
  'Museu da Moda', 'MUMO', 'Museu da Imagem e do Som', 'MIS BH', 'Noturno nos Museus',
  'Semana Nacional de Museus', 'Fundação Municipal de Cultura', 'Museus de BH', 'Cultura BH'
];

const MEDIUM_TAIL_KEYWORDS = [
  'programação museus centro bh', 'oficinas museu da moda bh', 'atividades mhab belo horizonte',
  'exposição mis bh', 'agenda cultural museus bh', 'programação viaduto das artes',
  'noturno nos museus programação', 'eventos culturais centro de bh', 'museus gratuitos belo horizonte',
  'atividades culturais prefeitura bh', 'oficinas gratuitas museus bh', 'programação museus municipais bh',
  'exposição museu da imagem e do som bh', 'agenda museu histórico abílio barreto', 'museu da moda programação'
];
...
const LONG_TAIL_KEYWORDS = [
  ...
  'experiências culturais gratuitas no centro de bh',
  'programação cultural fundação municipal de cultura bh',
  'oficinas e exposições museus centro abril 2026',
  'museus centro programação mês das mulheres bh',
  'semana nacional de museus museus centro bh',
  'viaduto das artes museus centro oficinas gratuitas',
  'museu histórico abílio barreto atividades educativas',
  'museu da moda belo horizonte programação gratuita'
];

const HASHTAGS = [
  '#MuseusCentro', '#ViadutoDasArtes', '#NoturnoNosMuseus', '#MuseuDaModa', '#MUMO', '#MHAB', '#MISBH',
  '#MuseusBH', '#MuseusDeBH', '#CulturaBH', '#CircuitoCultural', '#FundacaoMunicipalDeCultura',
  '#MuseuHistoricoAbilioBarreto', '#MuseuDaImagemEDoSom', '#ProgramacaoCulturalBH', '#EventosBH',
  '#ArteEmBH', '#OficinasMuseus', '#ExposicaoBH', '#PatrimonioCultural', '#CentroHistoricoBH',
  '#MemoriaBH', '#CulturaUrbanaBH', '#ArteEducacao', '#EducativoMuseu', '#MuseusMunicipais',
  '#AgendaBH', '#BeloHorizonteCultural', '#SemanaNacionalDeMuseus'
];

const SOCIAL_SEARCH_SOURCES = [
  'Instagram: @museuscentro, @viadutodasartes, @pbhcultura, hashtags do projeto',
  'Facebook: páginas dos museus, PBH Cultura, eventos e compartilhamentos',
  'YouTube: vídeos, entrevistas, shorts e transmissões sobre programação cultural BH',
  'TikTok: hashtags, vídeos curtos e menções espontâneas',
  'Threads/X: hashtags, links compartilhados e comentários',
  'Reddit: r/BeloHorizonte, recomendações culturais e discussões espontâneas',
  'Google Notícias e buscadores: imprensa, agendas culturais, portais institucionais'
];

const DRIVE_FOLDERS = [
  { id: '1ORE5fdfWe3WIhpVouB1Et6VLN2kVXFr8', name: 'Releases e Clipping', url: 'https://drive.google.com/drive/folders/1ORE5fdfWe3WIhpVouB1Et6VLN2kVXFr8', typeLabel: 'Releases e Clipping' },
  { id: '1kCcL0H7K2tLETDGo1sAs9LZ6UN_pLk4J', name: 'Imagens', url: 'https://drive.google.com/drive/folders/1kCcL0H7K2tLETDGo1sAs9LZ6UN_pLk4J', typeLabel: 'Imagens' },
  { id: '1WneHTmI8GYPMpdeumPNhIB9lzDiiArU_', name: 'Redes Sociais', url: 'https://drive.google.com/drive/folders/1WneHTmI8GYPMpdeumPNhIB9lzDiiArU_', typeLabel: 'Redes Sociais' },
];
...
const CLIPPING_ITEMS = [
  ...
  { id: '2026-01-arquivo-portal-bh', title: 'Arquivo de notícias Museus Centro - janeiro de 2026', sourceName: 'Portal Belo Horizonte', sourceType: 'Arquivo mensal', publishedDate: '2026-01-15', relevance: 'Média', platform: 'Site', relatedTo: ['Museus Centro', 'Arquivo histórico'], url: 'https://portalbelohorizonte.com.br/museuscentro/2025/noticias', summary: 'Link mensal para consulta de publicações anteriores sem limite temporal.' },
...
function detectMentions(item) {
  ...
  return allKeywordTerms().filter((keyword) => text.includes(normalizeText(keyword)) || (item.relatedTo || []).some((tag) => normalizeText(tag).includes(normalizeText(keyword))));
}
    ...
    <tr className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
      ...
      <td className="px-3 py-3 align-top">
        ...
        <div className="flex flex-wrap gap-1 mt-2">
          {mentions.slice(0, 5).map((tag) => <Badge key={tag} variant="outline" className="text-[10px] bg-white">{tag}</Badge>)}
        </div>
      </td>
      <td className="px-3 py-3 align-top">
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Painel de clipping sem limite temporal, redes sociais, hashtags e acervo de comunicação do projeto.</p>
        </div>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <KpiCard label="Publicações" value={clippingSummary.total} helper="sem limite temporal" icon={Newspaper} dark />
        ...
        <CardContent className="p-4 space-y-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">Lista aberta sem limite pretérito, agrupada por mês/ano, com busca por termos principais, cauda média, cauda longa e hashtags.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <div className="relative w-full sm:w-80">
                <Search className="absolute left-2.5 top-2.5 h-3.5 w-3.5 text-gray-400" />
                <Input className="h-9 pl-8 text-sm" placeholder="Buscar publicação, veículo, hashtag, cauda longa..." value={query} onChange={(event) => setQuery(event.target.value)} />
              </div>
                ...
                <section key={group.key} className="space-y-2">
                  ...
                  <div className="overflow-x-auto rounded-xl border border-gray-200">
                    <table className="w-full min-w-[860px] table-fixed border-collapse text-sm">
                      <colgroup><col className="w-[10%]" /><col className="w-[36%]" /><col className="w-[18%]" /><col className="w-[11%]" /><col className="w-[11%]" /><col className="w-[8%]" /></colgroup>
                      <thead>
      ...
      <Card className="rounded-2xl border-gray-200 bg-white shadow-sm">
        <CardContent className="p-4 space-y-4">
          <div><h2 className="text-lg font-semibold text-black">Acervo de comunicação</h2><p className="text-xs text-gray-500 mt-1">Pastas de referência no Google Drive.</p></div>
