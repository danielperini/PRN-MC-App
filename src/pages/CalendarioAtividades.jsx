...
function CalendarioAtividadesInner() {
  ...
  const todasAtividades = useMemo(() => {
    ...
    return mapItems(combinadas).sort(
  ...
  const salvar = async () => {
    ...
    try {
      ...
      await refetch();
      alert(
        res?.data?.message || 'Salvo e sincronizado com sucesso'
      );
      setShowEditor(false);
    } catch (e) {
