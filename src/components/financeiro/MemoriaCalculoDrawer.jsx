...
import { X, Calculator, ChevronUp, ChevronDown, Pencil } from 'lucide-react';
...
export default function MemoriaCalculoDrawer({ open, onClose, itens = [], previsto, utilizado, saldo, percentual, divergencia,   isCoordGeral = false, grupoPadrao }) {
  ...
  function handleEditar(rubrica) {
    // Pré-seleciona o grupo padrão do card quando a rubrica não tem grupo definido
    const rubricaComGrupo = (!rubrica.grupo && grupoPadrao) ? { ...rubrica, grupo: grupoPadrao } : rubrica;
    setRubricaEditando(rubricaComGrupo);
  }

  function handleEditSuccess() {
    queryClient.invalidateQueries({
      predicate: (q) => String(q.queryKey?.[0] || '').toLowerCase().includes('rubrica'),
    });
  }
  ...
  return (
    <>
        ...
        <div className="relative w-full max-w-4xl bg-white h-full flex flex-col shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <div className="flex items-center gap-2">
              ...
              {isCoordGeral && (
                <span className="ml-2 text-[10px] uppercase tracking-wide text-gray-400 border border-gray-200 rounded-full px-2 py-0.5">
                  modo edição
                </span>
              )}
            </div>
            ...
            <table className="w-full text-xs">
              <thead className="sticky top-0 bg-gray-50 border-b border-gray-200 z-10">
                <tr>
                  ...
                  {isCoordGeral && <th className="text-right px-3 py-2.5 font-semibold text-gray-700">Ações</th>}
                </tr>
              </thead>
              <tbody>
                {sorted.map((r, i) => {
                  ...
                  const sal = prev - util;
                  return (
                    <tr key={r.id || i} className={`group border-b border-gray-50 hover:bg-gray-50 transition-colors ${sal < 0 ? 'bg-red-50' : ''}`}>
                      ...
                      {isCoordGeral && (
                        <td className="px-3 py-2 text-right">
                          ...
                          >
                            <Pencil className="w-3.5 h-3.5 text-gray-400 hover:text-black" />
                          </button>
                        </td>
                      )}
                    </tr>
          ...
          <div className="border-t border-gray-200 bg-gray-50 px-4 py-3">
            <table className="w-full text-xs">
              <tfoot>
                <tr>
                  <td className="px-4 py-1 font-bold text-black" colSpan={isCoordGeral ? 5 : 4}>
      ...
      {isCoordGeral && rubricaEditando && (
        <EditRubricaDialog
          isOpen={!!rubricaEditando}
