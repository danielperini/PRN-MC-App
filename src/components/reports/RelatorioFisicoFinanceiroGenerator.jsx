...
function normalizePhotoMatchText(value = '') {
  return String(value || '')
    ...
    .replace(/[“”"']/g, '')
    .replace(/[_\-–—/:;,.!?()[\]{}]+/g, ' ')
    .replace(/\b(ati|img|jpeg|jpg|png|webp|foto|fotos|registro|fotografico|fotografica)\b/g, ' ')
    .replace(/\b\d{6,}\b/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}

function photoTextForActivityMatch(photo = {}) {
  return normalizePhotoMatchText([
    photo?.fileName,
    photo?.file_name,
    photo?.name,
    photo?.nome_arquivo,
    ...
    photo?.linkedActivity?.titulo,
  ].filter(Boolean).join(' '));
}

function getActivityMatchTerms(activity = {}) {
  const title = normalizePhotoMatchText(activity?.nome || activity?.titulo || activity?.nome_atividade || '');
...
function wordMatchesActivityKeyword(textWords = [], keyword = '') {
...
function photoHasDirectActivityLink(photo = {}, activity = {}) {
  const activityId = String(activity?.id || activity?._id || activity?.activity_id || activity?.atividade_id || '');
  ...
  return [
    ...
    photo?.activityId,
    photo?.atividadeId,
    photo?.linkedActivity?.id,
    photo?.linkedActivity?._id,
  ].filter(Boolean).some((id) => String(id) === activityId);
}

function photoLooksLinkedToActivity(photo = {}, activity = {}) {
  if (photoHasDirectActivityLink(photo, activity)) return true;

  const text = photoTextForActivityMatch(photo);
  const { title, compact, keywords } = getActivityMatchTerms(activity);
  if (!text || !title || keywords.length === 0) return false;

  const textCompact = text.replace(/\s+/g, '');
  if (compact.length >= 12 && textCompact.includes(compact.slice(0, Math.min(compact.length, 32)))) {
    return true;
  }

  const textWords = text.split(' ').filter(Boolean);
  const hits = keywords.filter((keyword) => wordMatchesActivityKeyword(textWords, keyword));
...
function buildPhotoSelectionCandidates(contexto = {}) {
  return (Array.isArray(contexto?.atividades) ? contexto.atividades : [])
    .map((atividade, index) => {
      const fotosVinculadas = (Array.isArray(atividade?.fotos) ? atividade.fotos : [])
        .filter((photo) => photoLooksLinkedToActivity(photo, atividade));

      const photos = fotosVinculadas
