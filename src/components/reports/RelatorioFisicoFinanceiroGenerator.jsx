...
export default function RelatorioFisicoFinanceiroGenerator() {
  ...
  async function buildLocalReport() {
    const [
      ...
      base44.entities.Communication
        ? safeList(base44.entities.Communication, '-created_date', 1000)
        : Promise.resolve([]),
    ]);
    ...
    const comprasSolicitacoes = filterPeriod(comprasRaw, dateFrom, dateTo).filter(
      (c) =>
        !museu ||
        c?.museu === museu ||
    ...
    const pagamentosEquipe = filterPeriod(teamPaymentsRaw, dateFrom, dateTo).filter(
      (c) =>
        !museu ||
        c?.museu === museu ||
        c?.centro_custo === museu
    ...
    const documentos = filterPeriod(documentosRaw, dateFrom, dateTo);
