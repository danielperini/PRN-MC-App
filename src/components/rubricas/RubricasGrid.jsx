...
export default function RubricasGrid({
  ...
  const grupos = useMemo(() => {
    return [...new Set(rubricasAtivas.map(r => r?.grupo).filter(Boolean))].sort();
  }, [rubricasAtivas]);
  ...
  const filtradas = useMemo(() => {
    return rubricasAtivas.filter(r => {
      ...
      const matchSearch =
        !searchTerm || nomeRubrica.includes(searchTerm);

      const matchGrupo =
        filtroGrupo === 'all' || r?.grupo === filtroGrupo;
  ...
  }, [rubricasAtivas, search, filtroGrupo, filtroStatus]);
