      ...
      <div className={`border rounded-lg p-6 ${getStatusClass(rubrica.percentual_utilizado || 0)}`}>
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="text-right">
            <p className="text-3xl font-bold text-black">{(rubrica.percentual_utilizado || 0).toFixed(2)}%</p>
      ...
      {showForm && (
        <div className="border border-gray-200 rounded-lg p-6 space-y-4 bg-gray-50">
          ...
          <div className="flex gap-2 justify-end pt-2 border-t">
            ...
            <Button className="bg-black hover:bg-gray-800 text-white" onClick={handleAddLancamento}>
              Adicionar
            </Button>
          </div>
        </div>
      ...
      <div>
        ...
        {lancamentos.length === 0 ? (
          ...
          <div className="space-y-3">
            {lancamentos.map(l => (
              <div key={l.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    {l.observacao && <p className="text-xs text-gray-600 italic mt-2">💬 {l.observacao}</p>}
                  </div>

                  <div className="text-right flex items-start gap-2">
                    <div>
                      <p className={`text-lg font-bold ${l.valor < 0 ? 'text-red-600' : 'text-blue-600'}`}>
