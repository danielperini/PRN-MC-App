...
function normalizeForm(data) {
  ...
  return {
    ...
    cronograma_parcelas: Array.isArray(data?.cronograma_parcelas)
      ? data.cronograma_parcelas
      : [],
...
function getEquipeFinanceRules(funcao, userName) {
  ...
  const found = rules.find((rule) =>
    rule.match.some((term) => f.includes(term))
  );
  ...
  return {
    ...
    data_inicio_contrato: isAnaLuiza
      ? ANA_LUIZA_START_DATE
      : DEFAULT_START_DATE,
  ...
  return (
    budgetLines.find((bl) => {
      ...
      const text = normalizeText(
        `${bl.codigo || ''} ${bl.nome || ''} ${bl.descricao || ''}`
      );
      return rule.rubricaKeywords.some((keyword) =>
...
export default function TeamMemberForm({
  ...
  const isEditingOwnData =
    editingMember &&
    currentUser &&
    editingMember.user_email === currentUser.email;
  ...
  const availableUsers = users.filter(
    (u) => !existingMembers.some((m) => m.user_email === u.email)
  );
  ...
  const preencherFormComTermo = (userEmail) => {
    ...
    if (!termoDoUsuario) return;

    const numeroParcelasTermo = Math.max(
      1,
      termoDoUsuario.parcelas?.length || 1
    );
    ...
    setForm((prev) => ({
      ...
      data_inicio_contrato:
        termoDoUsuario.data_inicio || prev.data_inicio_contrato,
      ...
      valor_parcela: valorTotalTermo
        ? valorTotalTermo / numeroParcelasTermo
        : prev.valor_parcela,
  ...
  const applyAutoFinanceByFuncao = (funcao, userName) => {
    ...
    setForm((prev) => {
      ...
      const chosenBudgetlineId =
        matchedBudgetLine?.id || prev.budgetline_id || '';
      ...
      return {
        ...
        data_inicio_contrato:
          prev.data_inicio_contrato || rule.data_inicio_contrato,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const numeroParcelas = Math.max(
        1,
        parseInt(form.numero_parcelas, 10) || 1
      );
      const parcelasPagas = Math.max(
        0,
        parseInt(form.parcelas_pagas, 10) || 0
      );
      const valorTotal = toNumber(form.valor_total);
      const valorParcela = valorTotal / numeroParcelas;
      const chosenBudgetlineId =
        form.budgetline_id || form.budget_line_id || form.rubrica_id || '';
      ...
      const data = {
        ...
        data_criacao:
          form.data_criacao || new Date().toISOString().split('T')[0],
      ...
      if (form.contrato_url && memberId) {
        try {
          const driveRes = await base44.functions.invoke(
            'saveContractToDrive',
            {
          ...
          const fileName = `Contrato_${form.user_name
            ?.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}`;
          ...
          toast.success(
            '✅ Contrato vinculado e armazenado no Google Drive com sucesso!'
          );
        } catch (driveError) {
          toast.error(
            'Membro salvo, mas erro ao vincular contrato. Tente novamente.'
          );
        }
      }
                  ...
                  <Select
                    ...
                    onValueChange={(v) => {
                      ...
                      setForm((prev) => ({
                        ...
                        representante_legal_nome:
                          user?.representante_legal_nome || '',
                        representante_legal_cpf:
                          user?.representante_legal_cpf || '',
              ...
              <div className="space-y-4 border-t pt-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      set('representante_legal_nome', e.target.value)
                    }
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      set('representante_legal_cpf', e.target.value)
                    }
                    placeholder="000.000.000-00"
                  />
                </div>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) => set('cargo_representante', v)}
                  >
                    <SelectTrigger>
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const numeroParcelasExtraidas = Math.max(
                          1,
                          parseInt(extracted.numero_parcelas, 10) || 1
                        );
                        const valorTotalExtraido = toNumber(
                        ...
                        setForm((prev) => ({
                          ...
                          numero_parcelas:
                            numeroParcelasExtraidas || prev.numero_parcelas,
                          valor_parcela:
                            valorTotalExtraido && numeroParcelasExtraidas
                              ? valorTotalExtraido / numeroParcelasExtraidas
                              : prev.valor_parcela,
                          cronograma_parcelas:
                            extracted.cronograma_parcelas ||
                            prev.cronograma_parcelas,
                          ...
                          objeto_contrato:
                            extracted.objeto_contrato ||
                            prev.objeto_contrato,
                          descricao_contrato:
                            extracted.descricao_contrato ||
                            prev.descricao_contrato,
                        }));
            ...
            <div className="grid grid-cols-4 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    set('valor_total', parseFloat(e.target.value) || 0)
                  }
                  step="0.01"
                  min="0"
                />
              ...
              <div>
                ...
                <Input
                  value={parcelaCalculada ? parcelaCalculada.toFixed(2) : ''}
                  disabled
                  className="bg-gray-50"
                />
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 text-xs">
              ...
              <div className="rounded-lg bg-gray-50 p-3">
                ...
                <p className="font-semibold text-black">
                  R$ {saldoEstimadoContrato.toFixed(2)}
                </p>
              </div>
            </div>
            ...
            {form.cronograma_parcelas?.length > 0 && (
              <div>
                <Label className="flex items-center gap-1 mb-2">
                  ...
                  Cronograma de Parcelas (extraído pela IA)
                </Label>
          ...
          {availableBudgetLines.length > 0 && (
            <Section title="Rubrica Orçamentária">
              <div>
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Selecione a rubrica / linha orçamentária" />
                  </SelectTrigger>
                  <SelectContent>
                    {availableBudgetLines.map((bl) => (
                      <SelectItem key={bl.id} value={bl.id}>
                        {bl.codigo || bl.nome || 'Sem código'} —{' '}
                        {bl.nome ||
                          bl.descricao?.substring(0, 80) ||
                          'Sem descrição'}
                      </SelectItem>
                    ))}
                  </SelectContent>
                ...
                {form.budgetline_id && (
                  <p className="text-xs text-gray-500 mt-1">
                    Esta vinculação será usada no controle de saldo e pagamento
                    mensal.
                  </p>
                )}
              </div>
