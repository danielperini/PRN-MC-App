...
function getBudgetLineLabel(line) {
  ...
  const descricao =
    line?.descricao || line?.rubrica || line?.nome || 'Linha orçamentária';
...
export default function PurchaseFormDialog({
  ...
  const [mes, setMes] = useState(
    initialData?.mes_referencia ||
      prefill?.mes_referencia ||
  ...
  useEffect(() => {
    ...
    toast.warning(
      'Rubrica ou linha orçamentária anterior removida por incompatibilidade com o centro de custo.'
    );
  }, [
    centroSelecionado,
    form.rubrica_id,
  ...
  const handleRubricaChange = (value) => {
    ...
    if (value.startsWith('BL-')) {
      ...
      if (!isCentroCompativel(centroSelecionado, lineCentro)) {
        toast.error(
          'Essa linha orçamentária não pertence ao centro de custo selecionado.'
        );
        return;
      }
    ...
    const linkedBudgetLine = findCompatibleBudgetLineForRubrica(
      value,
      centroSelecionado
    );
  ...
  const analyzeWithAI = async () => {
    if (
      ...
      toast.error(
        'Preencha: descrição, meta, categoria e tipo antes de analisar.'
      );
      return;
    }
  ...
  const sugerirRubricaIA = async () => {
    ...
    try {
      ...
      const rubricaEncontrada = rubricas.find(
        (r) => r.id === suggestion.rubrica_id
      );
      ...
      if (
        !isCentroCompativel(
          centroSelecionado,
          getEntityCentro(rubricaEncontrada)
        )
      ) {
        setRubricaSugestao(null);
        return;
  ...
  const handleSave = async (submeter = false) => {
    ...
    if (form.rubrica_id) {
      ...
      if (!isCentroCompativel(centroSelecionado, getEntityCentro(rubrica))) {
        toast.error(
          'A rubrica selecionada não é compatível com o centro de custo.'
        );
        return;
      }
    }
    ...
    if (form.budgetline_id) {
      ...
      if (!isCentroCompativel(centroSelecionado, getEntityCentro(budgetLine))) {
        toast.error(
          'A linha orçamentária selecionada não é compatível com o centro de custo.'
        );
        return;
      }
    }
    ...
    try {
      ...
      if (isEditing) {
        created = await base44.entities.PurchaseRequest.update(
          initialData.id,
          payload
        );
      } else {
  ...
  const saldoDisponivel = selectedLine
    ? (selectedLine.saldo_inicial || 0) -
      (selectedLine.saldo_comprometido || 0)
    : null;
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between border-b bg-white p-6">
          <div>
            ...
            <p className="text-xs text-gray-500">
              3º Termo Aditivo — Museus Centro
            </p>
          </div>
        ...
        <div className="space-y-6 p-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => set('tipo_gasto', v)}
                >
                  <SelectTrigger>
            ...
            <div>
              ...
              {!centroSelecionado && (
                <div className="mt-2 rounded-lg border border-red-200 bg-red-50 p-2 text-xs text-red-700">
                  Selecione o centro de custo antes de escolher rubrica e
                  enviar a compra.
                </div>
              )}
            </div>
              ...
              {isFromActivity ? (
                <div className="flex h-9 items-center gap-2 rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-800">
                  ...
                  <span className="truncate">
                    {prefill?._activity_titulo ||
                      form.activity_id?.slice(0, 12) + '…'}
                  </span>
                </div>
              ) : (
                <Select
          ...
          <div className="space-y-3 rounded-xl border border-gray-100 bg-gray-50 p-4">
            ...
            {aiAnalysis && (
              ...
              >
                ...
                {aiAnalysis.alerta &&
                  aiAnalysis.meta_sugerida !== form.meta_id && (
                    <div className="flex items-start gap-2 rounded-md bg-amber-100 p-2">
                      <AlertTriangle className="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-amber-600" />
                      <p className="text-xs text-amber-800">
                        <strong>Sugestão da IA:</strong> Esta compra pode se
                        encaixar melhor em{' '}
                        <strong>{aiAnalysis.meta_sugerida}</strong>. Considere
                        alterar a meta selecionada.
                      </p>
                    </div>
                  )}
              </div>
          ...
          <div className="space-y-2">
            ...
            <Select
              value={
                form.rubrica_id ||
                (form.budgetline_id ? `BL-${form.budgetline_id}` : '')
              }
            ...
            >
              ...
              <SelectContent className="max-h-64">
                {rubricasFiltradas.length > 0 && (
                  <>
                    <div className="bg-gray-100 px-2 py-1.5 text-xs font-semibold text-gray-600">
                      Rubricas compatíveis com{' '}
                      {centroSelecionado || 'o centro selecionado'}
                    </div>
                ...
                {centroSelecionado &&
                  ...
                  budgetLinesFiltradas.length === 0 && (
                    <div className="px-2 py-2 text-xs text-gray-500">
                      Nenhuma rubrica ou linha compatível encontrada para este
                      centro.
                    </div>
                  )}
              </SelectContent>
        ...
        <div className="sticky bottom-0 flex items-center justify-between rounded-b-2xl border-t bg-gray-50 p-6">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              disabled={
                saving || !saldoOk || !hasOrcamentoVinculado || !centroSelecionado
              }
            >
