...
function MemberCard({
  ...
  const valorParcela =
    ...
    (parcelas > 0 ? toNumber(member?.valor_total) / parcelas : 0);

  const valorTotal =
    toNumber(member?.valor_total) ||
    (parcelas > 0 && valorParcela > 0 ? parcelas * valorParcela : 0);

  const saldo = Math.max(0, valorTotal - pagasNoContrato * valorParcela);

  const dataInicio = member?.data_inicio || member?.data_inicio_contrato;
  const dataFim = member?.data_fim || member?.data_fim_contrato;
    ...
    <div className="border p-4 rounded-xl space-y-3">
      <div className="flex justify-between items-start gap-3">
        <div>
          ...
          <p className="text-xs text-gray-500">{member?.funcao || member?.role || '—'}</p>
        </div>
  ...
  const refreshAll = async () => {
    try {
      ...
      await Promise.all([
        ...
        queryClient.refetchQueries({ queryKey: ['team-members-all'] }),
