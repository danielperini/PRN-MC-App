...
export default function TeamPaymentReview() {
  ...
  function resolveRubricaFromMember(payment) {
    const member = members.find((m) => String(m.user_email || '').trim().toLowerCase() === String(payment.user_email || '').trim().toLowerCase());
  ...
  function buildApproveChecklist(payment) {
    ...
    const memberMatch = members.find((m) => String(m.user_email || '').trim().toLowerCase() === String(payment.user_email || '').trim().toLowerCase());
    ...
    const checks = [
      ...
      },
      {
        ...
        detailOk: rubricaOption
          ...
          : (selectedRubricaId ? `Rubrica vinculada por ID: ${selectedRubricaId}.` : 'Rubrica não informada.'),
