...
export default function TeamPaymentReview() {
  ...
  const [savingRubricaByPayment, setSavingRubricaByPayment] = useState({});
  ...
  function hasRubricaVazia(payment) {
    return !String(payment?.rubrica_id || '').trim() && !String(payment?.rubrica_nome || '').trim();
  }

  function hasRubricaDraftChanged(payment) {
    const currentId = String(payment?.rubrica_id || '').trim();
    const currentNome = String(payment?.rubrica_nome || '').trim();
    const selectedId = String(getSelectedRubricaId(payment) || '').trim();
    const selectedNome = String(getSelectedRubricaNome(payment) || '').trim();

    if (!selectedId) return false;
    return currentId !== selectedId || currentNome !== selectedNome;
  }
  ...
  async function saveRubrica(payment) {
    if (savingRubricaByPayment[payment.id]) return;

    clearCardMessages(payment.id);
    setSavingRubricaByPayment((prev) => ({ ...prev, [payment.id]: true }));

    try {
      const payload = await ensureRubricaPersistida(payment);

      const successMessage = `Rubrica vinculada com sucesso: ${payload.rubrica_nome || payload.rubrica_id}.`;
      ...
      setSavingRubricaByPayment((prev) => ({ ...prev, [payment.id]: false }));
    }
  }
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => saveRubrica(payment)}
                    disabled={savingRubrica || !selectedRubricaId}
                  >
                    {savingRubrica ? 'Salvando...' : 'Salvar rubrica'}
                  </Button>
                </div>

                {hasRubricaVazia(payment) && (
                  <div className="text-xs text-amber-700 font-medium">
                    Este pagamento está sem rubrica vinculada. Selecione na lista e salve.
                  </div>
                )}
              </div>
            )}
