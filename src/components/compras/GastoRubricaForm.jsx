...
export default function GastoRubricaForm({ rubricaId, isOpen, onClose, onSuccess }) {
  ...
  const handleSubmit = async () => {
    if (!form.fornecedor_nome || !form.valor || !form.data_gasto) {
      toast.error('Preencha fornecedor, valor e data');
      return;
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Adicionar Gasto à Rubrica</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div>
            ...
            <Select value={form.fornecedor_nome} onValueChange={v => {
              const forn = fornecedores.find(f => f.nome === v);
              setForm(p => ({ ...p, fornecedor_nome: v, categoria: forn?.categoria || '' }));
            }}>
