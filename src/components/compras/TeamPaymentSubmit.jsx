...
function getMemberDataStatus(member, currentUser) {
  ...
  if (!resolveRubricaId(member)) missing.push('Rubrica');
...
export default function TeamPaymentSubmit({ userEmail }) {
      ...
      try {
        ...
        if (!cancelled && hydratedMember) {
          setMemberLocalPatch({
            ...
            rubrica_id: member.rubrica_id || hydratedMember.rubrica_id || '',
            rubrica_nome: member.rubrica_nome || hydratedMember.rubrica_nome || hydratedMember.rubrica || ''
          });
        }
      } catch (e) {
  ...
  const selectedRubricaId = useMemo(() => resolveRubricaId(effectiveMember), [effectiveMember]);
  const selectedRubricaNome = useMemo(() => resolveRubricaNome(effectiveMember), [effectiveMember]);
  ...
  async function saveManualMemberFields() {
    ...
    await base44.entities.TeamMember.update(effectiveMember.id, {
      ...
      rubrica_id: selectedRubricaId || effectiveMember.rubrica_id || '',
      rubrica_nome: selectedRubricaNome || effectiveMember.rubrica_nome || ''
    }).catch(() => null);
  }
  ...
  async function handleSubmit(e) {
    ...
    if (!selectedRubricaId) {
      toast.error('Este envio precisa estar vinculado a uma rubrica.');
      return;
    }
    ...
    try {
      ...
      try {
        ...
        const existente = pickBestExistingPayment(existing);
        ...
        if (existente && ['PAGO', 'APROVADO_COORD', 'AGUARDANDO_APROVACAO'].includes(String(existente.status || '').toUpperCase())) {
      ...
      try {
        ...
        if (existente) {
          created = await base44.entities.TeamPayment.update(existente.id, payload);
        } else {
