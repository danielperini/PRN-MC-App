...
function monthKey(value) {
  ...
  const iso = text.match(/(20\d{2})-(\d{2})/); if (iso) return `${iso[1]}-${iso[2]}`;
  const br = text.match(/\b(\d{1,2})[\/-](\d{1,2})[\/-](20\d{2})\b/); return br ? `${br[3]}-${String(br[2]).padStart(2, '0')}` : 'sem-mes';
}
function monthLabel(key) { if (key === 'sem-mes') return 'Sem mês identificado'; const [year, month] = key.split('-'); return new Intl.DateTimeFormat('pt-BR', { month: 'long', year: 'numeric' }).format(new Date(Number(year), Number(month) - 1, 1)); }
function fiscalKey(item) {
  const key = accessKey(item); if (key.length >= 44) return `chave:${key}`;
  const nf = invoiceNumber(item); const doc = supplierDoc(item); const value = invoiceValue(item).toFixed(2);
  if (nf && doc) return `nf:${nf}:${doc}`; if (nf && value !== '0.00') return `nf-valor:${nf}:${value}`;
  const d = payload(item); const url = d.arquivo_original_url || d.nota_fiscal_url || d.nf_pdf_url || item?.arquivo_original_url || item?.nota_fiscal_url;
  return url ? `url:${url}` : `id:${item?.id}`;
}
function isInvoice(item) {
  const d = payload(item); const r = ai(item); const name = normalize(d.file_name_final || d.file_name_original || item?.file_name_final || item?.file_name_original); const type = normalize(d.tipo_detectado || item?.tipo_detectado || r.tipo_documento);
...
function bankRows(records = []) {
  return records.flatMap((record) => (record?.lancamentos || []).map((launch, index) => ({ id: `${record.id || record.drive_file_id}-${index}`, date: launch.data || '', month: monthKey(launch.data || `${record.ano || ''}-${String(record.mes_num || '').padStart(2, '0')}`), description: launch.descricao || launch.historico || 'Lançamento sem descrição', value: amount(launch.valor) }))).filter((row) => row.value > 0 && isDebit((records.find((record) => String(row.id).startsWith(String(record.id || record.drive_file_id)))?.lancamentos || [])[Number(String(row.id).split('-').pop())] || {}));
}
function findMatch(invoice, rows, used) {
  const value = invoiceValue(invoice); const month = monthKey(invoiceDate(invoice)); const tokens = normalize(supplierName(invoice)).split(' ').filter((token) => token.length >= 4);
  const ranked = rows.filter((row) => !used.has(row.id) && Math.abs(row.value - value) <= TOLERANCE).map((row) => ({ row, score: (row.month === month ? 3 : 0) + tokens.filter((token) => normalize(row.description).includes(token)).length * 2 })).sort((a, b) => b.score - a.score);
  if (!ranked.length) return null; return { ...ranked[0].row, ambiguous: ranked.length > 1 && ranked[0].score === ranked[1].score };
}

export default function NotasDriveForaPrestacao() {
  const queryClient = useQueryClient(); const [search, setSearch] = useState(''); const [driveFolderId, setDriveFolderId] = useState(() => localStorage.getItem('auditoria:notas-drive-folder-id') || '');
  ...
  const syncMutation = useMutation({ mutationFn: async () => { if (!driveFolderId.trim()) throw new Error('Informe o ID da pasta do Google Drive.'); localStorage.setItem('auditoria:notas-drive-folder-id', driveFolderId.trim()); const response = await base44.functions.invoke('syncNotasFiscaisDrive', { folder_id: driveFolderId.trim() }); const result = response?.data || response || {}; if (result.success === false || result.ok === false) throw new Error(result.error || 'Falha ao sincronizar notas fiscais.'); return result; }, onSuccess: async (result) => { await queryClient.invalidateQueries({ queryKey: ['notas-drive-conciliacao-prestacao'] }); toast.success(`${result.importadas || 0} nota(s) importadas e ${result.existentes || 0} já existentes.`); }, onError: (error) => toast.error(`Sincronização do Drive: ${error?.message || error}`) });

  const summary = useMemo(() => {
    const accounted = new Set(purchases.map(fiscalKey)); const unique = new Map();
    intakes.filter(isInvoice).forEach((item) => { const status = normalize(item?.status_processamento); if (status.includes('arquivado') || status.includes('rejeitado')) return; const key = fiscalKey(item); const current = unique.get(key); if (!current || String(item?.updated_date || '') > String(current?.updated_date || '')) unique.set(key, item); });
    const missing = [...unique.entries()].filter(([key]) => !accounted.has(key)).map(([, item]) => item).sort((a, b) => invoiceValue(b) - invoiceValue(a)); const rows = bankRows(bank); const used = new Set();
    const reconciled = purchases.filter((item) => invoiceValue(item) > 0).map((invoice) => { const match = findMatch(invoice, rows, used); if (match && !match.ambiguous) used.add(match.id); return { invoice, match, status: !match ? 'sem_lancamento' : match.ambiguous ? 'ambiguo' : 'conciliado' }; });
    const byMonth = new Map(); missing.forEach((item) => { const key = monthKey(invoiceDate(item)); if (!byMonth.has(key)) byMonth.set(key, { key, label: monthLabel(key), count: 0, total: 0 }); const row = byMonth.get(key); row.count += 1; row.total += invoiceValue(item); });
    const total = missing.reduce((sum, item) => sum + invoiceValue(item), 0); const exactBankTarget = rows.filter((row) => Math.abs(row.value - TARGET) <= TOLERANCE); const exactInvoiceTarget = [...missing, ...purchases].filter((item) => Math.abs(invoiceValue(item) - TARGET) <= TOLERANCE);
  ...
  const filtered = useMemo(() => { const term = normalize(search); const value = amount(search); if (!term) return summary.reconciled; return summary.reconciled.filter(({ invoice, match }) => (value > 0 && (Math.abs(invoiceValue(invoice) - value) <= TOLERANCE || Math.abs(Number(match?.value || 0) - value) <= TOLERANCE)) || normalize([supplierName(invoice), invoiceNumber(invoice), match?.description].join(' ')).includes(term)); }, [summary.reconciled, search]);
  if (l1 || l2 || l3) return null;

  return <section className="space-y-5 rounded-2xl border border-amber-200 bg-white p-5 shadow-sm">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="flex items-center gap-2 text-base font-bold"><AlertTriangle className="h-5 w-5 text-amber-600" /> Reconciliação de notas, Drive e extratos</h2><p className="mt-1 text-xs text-slate-500">A competência bancária vem da data de cada lançamento, nunca do mês da pasta de importação.</p></div><div className={`rounded-xl border px-4 py-2 text-right ${summary.aggregateMatchesTarget ? 'border-red-400 bg-red-50' : 'border-amber-200 bg-amber-50'}`}><p className="text-[10px] uppercase text-slate-500">Total agregado de notas faltantes</p><p className="font-bold">{fmt(summary.total)}</p>{summary.aggregateMatchesTarget && <p className="text-[10px] font-bold text-red-700">R$ 44.350,03 é soma agregada, não uma nota individual</p>}</div></div>
