...
export default function TrendChart({ data, title, dataKey, type = 'line' }) {
  const { primary } = useThemeChartColors();
  const exportData = () => {
  ...
  return (
    <div className="border border-gray-100 rounded-2xl p-6 themed-card">
      ...
      <ResponsiveContainer width="100%" height={300}>
        {type === 'area' ? (
          <AreaChart data={data}>
            ...
            <Area type="monotone" dataKey={dataKey} stroke={primary} strokeWidth={2} fillOpacity={1} fill="url(#colorArea)" />
          </AreaChart>
        ) : (
          <LineChart data={data}>
            <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" vertical={false} />
            ...
            <YAxis tick={{ fontSize: 12 }} axisLine={false} tickLine={false} />
            <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8, border: '1px solid #e5e7eb' }} formatter={(v) => v.toLocaleString('pt-BR')} />
            <Line type="monotone" dataKey={dataKey} stroke={primary} strokeWidth={3} dot={{ r: 5 }} activeDot={{ r: 7 }} />
          </LineChart>
        )}
      </ResponsiveContainer>
    </div>
