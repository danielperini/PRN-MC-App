...
export default function FluxoCaixaMensal() {
  ...
  if (isLoading || movimentacoes.length === 0) return null;
      ...
      {expanded && (
        <div className="px-5 pb-6 border-t border-slate-100 space-y-5">
              ...
              <tbody>
                {dadosMensais.map(d => (
                  <tr key={d.key} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className={`py-2 px-2 text-right font-bold ${d.saldo >= 0 ? 'text-slate-800' : 'text-orange-600'}`}>
                      {fmtBRL(d.saldo)}
                    </td>
                  </tr>
                ))}
          ...
          <div className="flex justify-end">
            <Link to="/Movimentacoes" className="text-xs text-slate-500 hover:text-slate-800 underline underline-offset-2 transition-colors">
