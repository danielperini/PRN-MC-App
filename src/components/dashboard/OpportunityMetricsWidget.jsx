    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card className="p-6 border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-3xl font-bold text-black">{(metrics.totalOpp / Math.max(reports.length, 1)).toFixed(1)}</p>
              <p className="text-sm text-gray-500 mt-1">Oportunidades / Relatório</p>
            </div>
            <AlertCircle className="w-8 h-8 text-blue-400 opacity-30" />
          </div>
        </Card>
      </div>
      ...
      {metrics.chartData.length > 0 && (
        <Card className="p-4 border-gray-200">
          <p className="text-sm font-semibold text-black mb-4">Tendência de Oportunidades</p>
